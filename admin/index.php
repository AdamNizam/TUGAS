<html>
<head>
	<title>Admin Dinus Discuss</title>
	<link rel="icon" type="image" href="../images/logodinus.png">

	<!--Custom CSS-->
	<link rel="stylesheet" type="text/css" href="../css/global.css">
	<!--Bootstrap CSS-->
	<link rel="stylesheet" type="text/css" href="../css/bootstrap.css">
	<link rel="stylesheet" type="text/css" href="../css/bootstrap.min.css">

    <link href="//netdna.bootstrapcdn.com/font-awesome/4.0.3/css/font-awesome.css" rel="stylesheet">
    <!--Script-->
    <script src="../js/jquery.js"></script>
    <script src="../js/bootstrap.js"></script>
    <script src="../js/bootstrap.min.js"></script>
    <script src="../js/loginadmin.js"></script>
</head>
<body>
	<div class="images" alt="Responsive image" style="background-image: url('../images/backgrounddiscuss.jpg');">
	<div class="container" style="margin:10% auto;width:400px;">
		<div class="panel panel-primary">
			<div class="panel-heading">
			<h3 class="panel-title text-center">Admin</h3>
		</div>
			<div class="panel-body">
			<form method="POST" class="form-signin" id="lg-form" action="login.php">
				<input type="text" class="form-control"  name="uname" placeholder="Username" >
				<input type="password" class="form-control"   name="pwd" placeholder="Password" >
				<input type="submit" class="btn btn-primary"  value="Masuk" style="width:100%;">
			</form>
			</div>
			</div>
		</div>
	</div>
	<div class="footer container text-center">
			<h5>Copyright &copy; 2016 Dinus Discuss Team </h5>
	</div>
	</body>
</html>